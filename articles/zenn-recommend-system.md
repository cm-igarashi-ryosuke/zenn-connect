...
Zennではこれまで、いいね数やPV数などを基にした独自の計算によるランキング（Trending）を提供してきました。Zennの読者にはWeb開発系のユーザーが多く、Trendingにはそうした記事がランキング上位に表示されやすい傾向がありました。
...
通常、LLMによるベクトル化（embeddings）では、数百〜1000次元以上の高次元ベクトルが生成されますが、Zennではデータ量が少ないことと、人間が把握しやすいように、記事の特徴を6次元のベクトルで表現することにしました。これにより、計算コストを抑えつつ、記事の特徴を十分に表現できると考えました。
...
それぞれのカテゴリーについて値は0.00~1.00の範囲で持ちます。全て当てはまらない記事や、多くのカテゴリーに当てはまる記事があるため、ベクトルの正規化は行いませんでした。
