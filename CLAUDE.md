...
- **「—」（emダッシュ）を使用しない**: 区切りや補足には括弧や別の表現を使う。emダッシュはAI的な文体の特徴であり、日本語の技術記事では不自然
- **見出し（heading）でコロンを使ってタイトルをつながない**: `## パターン1: Workers Rate Limiting Binding` のように `:` で主タイトルと補足をつなぐ書き方は使わない。代わりに、補足は見出しの直下の本文で書く。ただし、`## パターン3. Durable Object` のように連番（`1.` `2.` `3.` など）の後ろにタイトルを書くのは例外として OK
